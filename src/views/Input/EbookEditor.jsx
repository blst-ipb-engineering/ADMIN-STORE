...
class EbookEditor extends Component {
    constructor(props) {
        super(props);
        this.state = {
            isFetching: false,
    ...
    onUpdateHandler = (event) => {
        ...
        this.setState({ prompt: false }, () => {
            ...
            ProductUpdateEbook(content).then(res => {
                if (res.status === "success") {
                    ...
                    this.props.history.replace('/dashboard/ebooks');
                    this.props.history.push('/dashboard/ebooks');
                }
    ...
    onSaveHandler = (event) => {
        this.setState({ prompt: false }, () => {
            event.preventDefault();
            const content = this.state;
            ProductAddEbook(content).then(res => {
                if (res.status === "success") {
                    const toaster = {
                        isOpenToast: true,
    ...
    componentDidMount() {
        ...
        if (this.props.match.params.status === "edit" || this.props.match.params.status === "duplicate") {
            ...
            ProductEditEbook(content).then(res => { 
                        ...
                        <Col md={8} xs={12}>
                            ...
                            <Card className="card-user">
                                ...
                                <CardBody>
                                    ...
                                    <Col md={12}>
                                        <Label for="name" required>URL<small>/ Link</small></Label>
                                        <Input type="text" value={this.state.url_ebook} name="url_ebook" onChange={(event) => this.setState({ url_ebook: event.target.value }, () => { this.countFilled() })}></Input>
                                    </Col>
                                    <Col md={12}>
                                        <ReactTooltip />
                            ...
                            <Row >
                                <Col md={12} style={{ textAlign: 'right' }}>
                                    {this.state.saveable && this.state.sumFilled > 30 && this.state.thumbnailFile.length > 0 ? (
                                        <div>
                                            <Button onClick={(event) => this.props.history.push('/dashboard/ebooks')} color="secondary">Cancel</Button>
...
export default connect(mapStateToProps, mapDispatchToProps)(EbookEditor);
