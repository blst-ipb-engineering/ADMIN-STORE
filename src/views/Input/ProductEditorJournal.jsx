...
class ProductEditorJournal extends Component {
    constructor(props) {
        super(props);
        this.state = {
            isFetching: false,
    ...
    onUpdateHandler = (event) => {
        ...
        this.setState({ prompt: false }, () => {
            ...
            ProductUpdateJournal(content).then(res => {
                if (res.status === "success") {
                    ...
                    this.props.history.replace('/dashboard/product-jurnal');
                    this.props.history.push('/dashboard/product-jurnal');
                }
    ...
    onSaveHandler = (event) => {
        this.setState({ prompt: false }, () => {
            event.preventDefault();
            const content = this.state;
            ProductAddJournal(content).then(res => {
                if (res.status === "success") {
                    const toaster = {
                        isOpenToast: true,
    ...
    componentDidMount() {
        ...
        if (this.props.match.params.status === "edit" || this.props.match.params.status === "duplicate") {
            ...
            ProductEditJournal(content).then(res => { 
                ...
                Promise.all([push_cat, push_picture, push_blst_employee]).then(() => {
                ...
                <form>
                    <Row>
                        ...
                        <Col md={8} xs={12}>
                            ...
                            <Card className="card-user">
                                <CardHeader>
                                    <h6>Product Information Jurnal</h6>
                                </CardHeader>
                                <CardBody>
                                    <Col md={12}>
                                        <Label for="name" required>Journal Name <small>/ Nama Jurnal</small></Label>
                                    ...
                                    <Col md={12}>
                                        <Label for="name" required>Akreditasi</Label>
                                        <Input type="text" value={this.state.akreditasi} name="akreditasi" onChange={(event) => this.setState({ akreditasi: event.target.value }, () => { this.countFilled() })}></Input>
                                    </Col>
                            ...
                            <Card className="card-user">
                                ...
                                <CardBody style={{ minHeight: "0" }}>
                                    <Row>
                                        <Col md={7}>
                                            <Label for="name" required><strong>Price</strong><small>/ Harga</small></Label>
                                            <InputGroup>
                        ...
                        <Col md={6}>
                            ...
                            <Card className="card-user">
                                <CardHeader>
                                    <h6>Journal Detail <small>/ Detail Jurnal</small></h6>
                                </CardHeader>
                                <CardBody>
                                    <Row>
                                        <Col md={12}>
                                    ...
                                    <Row>
                                        <Col md={12}>
                                            <Label for="name">P-ISSN <small>/ P-ISSN</small></Label>
                                            <Input value={this.state.isbn} type="text" name="isbn" onChange={(event) => this.setState({ isbn: event.target.value }, () => { this.countFilled() })}></Input>
                                        </Col>
                                    </Row>

                                    <Row>
                                        <Col md={12}>
                                            <Label for="name">E-ISSN <small>/ E-ISSN</small></Label>
                            ...
                            <Row >
                                <Col md={12} style={{ textAlign: 'right' }}>
                                    {this.state.saveable && this.state.sumFilled > 30 && this.state.thumbnailFile.length > 0 ? (
                                        <div>
                                            <Button onClick={(event) => this.props.history.push('/dashboard/product-jurnal')} color="secondary">Cancel</Button>
...
export default connect(mapStateToProps, mapDispatchToProps)(ProductEditorJournal);
